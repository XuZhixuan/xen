...
RUN \
    ...
    echo -e "echo \"TEST\"\n" >> $RCS_FILE && \
    echo -e "echo 0 > /sys/devices/platform/passthrough/e000000.fakedev-irq/trigg\n" >> $RCS_FILE && \
    echo -e "echo 1 > /sys/devices/platform/passthrough/e000000.fakedev-irq/trigg\n" >> $RCS_FILE && \
    echo -e "if [ \"\$(cat /proc/interrupts | grep 'fakedev-irq' | awk '{ if (\$2 == 1) print }' | wc -l)\" -eq 2 ]; then\n" >> $RCS_FILE  && \
    echo -e "    echo \"AIA IS OK\"\n" >> $RCS_FILE && \
    echo -e "else\n" >> $RCS_FILE && \
    echo -e "    echo \"NOT OK\"\n" >> $RCS_FILE && \
    echo -e "fi\n" >> $RCS_FILE && \
    echo -e "/bin/sh\n" >> $RCS_FILE && \
